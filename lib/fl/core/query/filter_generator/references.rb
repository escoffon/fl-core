...
module Fl::Core::Query::FilterGenerator
  ...
  class References < Base
    ...
    def generate_simple_clause(name, desc, value)
      ph = Fl::Core::Query::FilterHelper.normalize_lists_of_references(value, desc[:class_name])
      if desc[:generator]
    ...
    # {Fl::Core::Query::FilterHelper.convert_list_of_references} and installs its return value as the new value.
    #
    # @param name [Symbol] The name of the filter. This is the value of a key in the *body* argument to
    #  {Fl::Core::Query::Filter#generate}. It is also the value of a key in the **:filter** option of the
    #  filter configuration passed to {Fl::Core::Query::Filter#initialize}.
    ...
    def normalize_value(name, desc, value)
      ...
      if nv.has_key?(:only)
        nv[:only] = Fl::Core::Query::FilterHelper.convert_list_of_references(nv[:only], desc[:class_name])
      end

      if nv.has_key?(:except)
        nv[:except] = Fl::Core::Query::FilterHelper.convert_list_of_references(nv[:except], desc[:class_name])
      end
