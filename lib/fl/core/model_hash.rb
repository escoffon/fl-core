module Fl::Core
  # A concern to provide mixins for converting an object to a hash.
  # The API implemented by this concern introduces the notion of an _actor_: this is the entity that makes
  ...
  module ModelHash
    ...
    def to_hash(actor, opts = nil)
      keys = to_hash_expand_key_list(actor, opts)
      rv = to_hash_base(actor, keys[:keys], keys[:options])
      ...
      to_hash_local(actor, l_keys, keys[:options]).each do |k, v|
        rv[k] = v
      end
    ...
    # Generate the list of properties to hash and merged options.
    # This method returns a Hash that contains two key/vaue pairs:
    #
    #  - **:keys** is an array that contains the names of attributes to return in the model hash.
    #    This is a superset of the value of the *keys* parameter to {#to_hash_local}: it includes the default
    #    keys like **:id** and **:type** that are already loaded in the model hash before {#to_hash_local} is
    #    called.
    #  - **:options** is a hash that contains the merged hash options.
    #    This is the value of the *opts* parameter to {#to_hash_local}.
    #
    # See {#to_hash} and {#to_hash_local} for details.
    #
    # @return [Hash] Returns a hash containing the two keys listed above.

    def to_hash_expand_key_list(actor, opts = nil)
      opts = opts || {}
      ...
      return { keys: c_keys, options: n_opts }
    end
    ...
    #  {Fl::Core::ModelHash#to_hash_local}.
    ...
    #
    #  @return [Hash] Returns a hash containing #to_hash options.
    #  If you plan on modifying the value, you might want to make a copy first, so that the original is not affected.

    # @!method to_hash_options_for_key(key, opts, defaults = nil)
    #  @scope class
    #  Get the hash options for a given key.
    #  This method looks up +key+ in the **:to_hash** value in +opts+, and returns its value if
    #  present. Otherwise, it returns +defaults+ if provided. Finally, it returns an empty hash.
