...
const { FlCoreComment } = require('fl/core/comment');
...
let <%=@api_service_class_name%> = FlClassManager.make_class({
	 ...
	 * @param {Object|String} contents_delta The comment contents, in Quill Delta format. A string value is
	...
	add_comment: function(commentable, contents, contents_delta, title, author, config) {
	    let cp = {
		commentable: (_.isString(commentable)) ? commentable : commentable.fingerprint,
		contents: contents,
		contents_delta: (_.isObject(contents_delta)) ? JSON.stringify(contents_delta) : contents_delta
	    };
...
FlGlobalAPIServiceRegistry.register('fl.services', { <%=@api_service_class_name%>: 'Fl::Core::Comment' });
